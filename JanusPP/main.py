...
for d_level in d_level_arr: #选择对应的d
    for file_data in file_data_arr:
        ...
        with open(filename, "r") as f:        
                for line in f:
                    values = line.split(" ")
                    universal_dict[values[0]] = {str(x) for x in values[1:len(values)]} #所有的关键字对应文件
        
        cur_del_no = num_del_arr[d_level_arr.index(d_level)]  #根据文件总数，选取删除的个数数组

        for num_del in cur_del_no:   #选取删除文件的个数
            del_array = original_del[:num_del] #选取删除的文件
            
            # initialise SSE_Client        
            client = SSE_Client(iv) #创建对象
            print("")
            print(">>load: d= " + str(d_level) + " matched ids= " + str(file_data) + " num_del= " + str(num_del)) 
            client.initDict(universal_dict) #传入文件数组
             
            print(">>>>>> encryption")
            start = timer()
            client.enc(d_level)  #把d传进去  加密文件
            end = timer()
            print(">>>>>> time taken " + str((end - start)*1000) + " ms")
            
            print(">>>>>> deletion")
            start = timer()            
            for i in del_array: #删除文件数组
                client.delfileId('Subject', i)  #删除文件
            end = timer()
            print(">>>>>> time taken " + str((end - start)*1000) + " ms")
            ...
            keyword = 'Subject'
            token = client.token_generation(keyword) #enc_keyword
