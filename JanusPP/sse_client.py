...
class SSE_Client:
    ...
    def enc(self,d):
        ...
        self.encrypted_index.clear()   #清空加密索引
        iv = self.fixed_iv
        d = d +1 # to increase the root node  0-d
        for keyword in self.lookUp: #选择关键字

            #hash keyword first
            enc_keyword = self.crypto_primitives_hmac(self.SK0, bytes(keyword,'utf-8')) #this is used to encrypt the keyword F(SK0, keyword)
            encrypted_identifiers = set([])
            
            #generate sk series
            key_series = [0 for i in range(d)]
            sk = self.encrypt(self.keytrim(self.SK0),keyword, iv) #sk = AES(SK0, keyword) 作为起始sk0
            key_series[0] = sk
            for i in range(1,d): #从1-d
                next_sk = self.crypto_primitives_hmac(self.SKI,key_series[i-1]) #计算所有的ski F(SK1,ski-1)
                key_series[i] = next_sk          
                
            #store the key_series for the keyword
            self.keyword_keyseries[keyword] = key_series #所有ski
            
            identifers = self.lookUp[keyword]   #所有文件号
            for identifier in identifers:
                ...
                for key_item in key_series:                          
                        result =self.encrypt(self.keytrim(key_item),t_id[0], iv) #AES(sk0,t_id[0]) #AES(sk1,t_id[0]) ……
                        result =self.encrypt(self.keytrim(result),t_id[1],iv)  #AES(AES(sk0,t_id[0]),t_id[1] )
                        ...
                        int_result = int.from_bytes(result, sys.byteorder) #转换为int
                        k_id = k_id ^ int_result #异或求得k=xor F(ski,t)

                #once we have k_id then we do hash as Cash paper to get encrypted keyword               
                encrypted_id = k_id^int(identifier)       #SE.Enc(k,m)         回头给改成AES加密。。。。。。
                encrypted_identifiers.add((encrypted_id,t_id))
              
            #then we add it into a tuple, finally we write to the file this dictionary
            self.encrypted_index[enc_keyword] =  encrypted_identifiers  #enc_keyword 是标志符 后面跟着加密索引

    def delfileId(self, keyword, fileid):
        
        enc_keyword = self.crypto_primitives_hmac(self.SK0, bytes(keyword,'utf-8')) #求enc_keyword 关键字标志符
        path_tuples = []
        keyseries = self.keyword_keyseries[keyword] #sk0 sk1 sk2 sk3
        iv = self.fixed_iv
         
        t_id = self.string2HashedBinary(fileid) #删除文件对应的哈希值
             
        key_initiator = ''       
        if enc_keyword not in self.keyword_stage: #if the keyword has not been used in deletion
            key_initiator = keyseries[0] #sk0
        else:
        ...
        for b_index in range(len(t_id)):
            if len(traveled)==0:
                if t_id[b_index] == '0':
                    result =self.encrypt(self.keytrim(key_initiator),'1', iv)  #AES(sk0, '1')
                    i_tuple= (result,'1')
        ...
        if enc_keyword not in self.keyword_stage:
            self.keyword_stage[enc_keyword] = (keyseries[1],1) #(sk1 ,1)
        else:
    ...
    def string2HashedBinary(self, msg): #传入msg 哈希过后从256位转换为16位的数据
        
        msg_sign= bytes(msg,'utf-8')
        hashcode=hashlib.sha256(msg_sign).hexdigest() #求哈希
        binary = lambda x: "".join(reversed( [i+j for i,j in zip( *[ ["{0:04b}".format(int(c,16)) for c in reversed("0"+x)][n::2] for n in [1,0] ] ) ] ))
        xor = lambda x,y: ''.join([_xormap[a, b] for a, b in zip(x, y)])
        
        bin_str = binary(hashcode) #转换为二进制 256位
